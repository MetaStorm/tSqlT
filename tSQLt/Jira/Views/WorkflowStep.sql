...
WITH WS AS (SELECT DISTINCT Workflow, StepFrom AS Step, IsVirtual
    ...
    SELECT        ISNULL(Workflow, '') AS Workflow, Step, ISNULL(CAST(IsVirtual AS bit), 0) AS IsVirtual
     FROM            WS AS WS_1
GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'Jira', @level1type = N'VIEW', @level1name = N'WorkflowStep';
...
Begin DesignProperties = 
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'Jira', @level1type = N'VIEW', @level1name = N'WorkflowStep';
